...
class Server(commands.Cog):
    ...
    async def set_prefix(self, ctx: Context, new_prefix: str = None):
        ...
        try:
            ...
            message = f"**Server prefix** updated to `{new_prefix}`"
            embed = make_embed_success(ctx.author, message)
            await ctx.send(embed=embed)
        except Exception as e:
            logger.error(f"An error occurred in Server set_prefix: {e}")
                ...
                async with conn.cursor(aiomysql.DictCursor) as cur:
                    ...
                    if result:
                        ...
                        if prefix_exists:
                            ...
                            message = f"Your server's prefix has been **removed**. You can set a **new prefix** using `@Sparky#1871 prefix add <prefix>`"
                            embed = make_embed_success(ctx.author, message)
                            await ctx.send(embed=embed)
                        else:
                            message = "Your server doesn't have a **prefix set**! Set it using `@Sparky#1871 prefix add <prefix>`"
                            embed = make_embed_warning(ctx.author, message)
                            await ctx.send(embed=embed)

        except Exception as e:
            logger.error(f"An error occurred in Server remove_prefix: {e}")

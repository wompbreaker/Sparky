...
#######################################################################################################
    ...
    async def setup_jailed_perms(self, ctx: commands.Context):
    ...
    async def jail_member(self, ctx: commands.Context, member: Optional[discord.Member]):
        ...
        try:
            ...
            message = f"An error has occurred in {self.qualified_name} database jailed check: {e}"
            error_embed = make_embed_error(ctx.author, message)
        ...
        try:
            ...
            if jail_channel_exists is False:
                warning_message = f"No **jail channel** was found, please run the `{cmd_prefix}setup` command first"
                warning_embed = make_embed_warning(ctx.author, warning_message)
                await ctx.send(embed=warning_embed)
                return
            ...
            if member == ctx.author:
                message = "You can't **jail** yourself."
                embed = make_embed_error(ctx.author, message)
                await ctx.send(embed=embed)
                return

            if member == ctx.guild.owner:
                message = "You can't **jail the owner**."
                embed = make_embed_error(ctx.author, message)
                await ctx.send(embed=embed)
                return
            ...
            message = f"Successfully **jailed {member}**"
            embed = make_embed_success(ctx.author, message)
            await ctx.send(embed=embed)
    ...
    async def jailmsg(self, ctx: commands.Context):
    ...
    @commands.has_guild_permissions(manage_guild=True)
    async def set(self, ctx: commands.Context, *, jail_message: typing.Optional[str]):
        ...
        try:
            async with self.bot.pool.acquire() as conn:
                async with conn.cursor() as cur:
                    ...
                    if is_already_setup:
                        ...
                        await conn.commit()
                        message = f"Jail message changed to **{jail_message}**"
                        embed = make_embed_success(ctx.author, message)
                        await ctx.send(embed=embed)
                    else:
                        cmd_prefix = await self.bot.bot_prefix(ctx.guild)
            ...
            logger.error(f"Failed in jailmsg set: {e}")
    ...
    @commands.bot_has_permissions(manage_roles=True)
    @commands.has_permissions(manage_messages=True)
    async def unjail_member(self, ctx: commands.Context, member: Optional[discord.Member]):
        ...
        try:
            ...
            if role_check:
                ...
                message = f"{member.mention} isn't **jailed**"
                embed = make_embed_warning(ctx.author, message)
                await ctx.send(embed=embed)
                return

            message = f"Successfully **unjailed {member}**"
            embed = make_embed_success(ctx.author, message)
            await ctx.send(embed=embed)
...
#######################################################################################################
    ...
    async def create_channel(self, ctx: commands.Context, channel_type: Optional[str], channel_count: typing.Optional[int] = 1):
        ...
        if channel_type not in channel_types:
            message = f"**{channel_type}** is not a proper channel type."
            embed = make_embed_warning(ctx.author, message)
            await ctx.send(embed=embed)
            return
        else:
            for batch in range(channel_count):
                ...
                message = f"Successfully created {channel_count} {channel_type} channels"
                embed = make_embed_success(ctx.author, message)
                await ctx.send(embed=embed)
                return

    async def create_chan(self, ctx: commands.Context, channel_type: str):
        channel = ''
        try:
            ...
            message = f"Failed to create a channel {e}"
            embed = make_embed_error(ctx.author, message)
            await ctx.send(embed=embed)
            return
    ...
    async def lock_channel(self, ctx: commands.Context, channel: Optional[discord.abc.GuildChannel]):
        """Lockdown a channel"""
        ...
        try:
            if not isinstance(channel, discord.CategoryChannel):
                ...
                message = "You can't lock a category channel"
                embed = make_embed_warning(ctx.author, message)
                await ctx.send(embed=embed)
    ...
    async def lock_role(self, ctx: commands.Context, role: Optional[discord.Role]):
        """Set the default lock role"""
        try:
            if role is None:
                await self.set_lock_role_id(ctx.guild, None)
                message = "Removed the **default lock role**"
                embed = make_embed_warning(ctx.author, message)
                await ctx.send(embed=embed)
                return

            await self.set_lock_role_id(ctx.guild, role.id)
            message = f"{role.mention} is now set as the **default lock role**"
            embed = make_embed_success(ctx.author, message)
            await ctx.send(embed=embed)
        except Exception as e:
            logger.error(f"Error in {self.qualified_name} lock_role: {e}")
            message = f"An error occurred while setting the lock role: {e}"
            error_embed = make_embed_error(ctx.author, message)
            await ctx.send(embed=error_embed)
    ...
    @commands.has_permissions(manage_guild=True)
    async def lock_ignore(self, ctx: commands.Context):
    ...
    @commands.has_permissions(manage_guild=True)
    async def lock_ignore_add(self, ctx: commands.Context, channel: Optional[discord.abc.GuildChannel]):
        ...
        try:
            ...
            message = f"{channel.mention} is now an **ignored channel**"
            embed = make_embed_success(ctx.author, message)
            await ctx.send(embed=embed)
        except Exception as e:
            logger.error(f"Error in Channel lock_ignore_add: {e}")
            await ctx.message.add_reaction("👎")
    ...
    @commands.has_permissions(manage_guild=True)
    async def lock_ignore_remove(self, ctx: commands.Context, channel: Optional[discord.abc.GuildChannel]):
        ...
        try:
            ...
            if channel.id not in ignored_channels:
                message = f"{channel.mention} is not configured as an **ignored channel**"
                embed = make_embed_warning(ctx.author, message)
                await ctx.send(embed=embed)
                return
            await self.remove_ignored_channel(ctx.guild, channel)
            message = f"{channel.mention} is no longer an **ignored channel**"
            embed = make_embed_success(ctx.author, message)
            await ctx.send(embed=embed)
        except Exception as e:
            logger.error(f"Error in Channel lock_ignore_remove: {e}")
            await ctx.message.add_reaction("👎")
    ...
    @commands.has_permissions(manage_guild=True)
    async def lock_ignore_list(self, ctx: commands.Context):
        """View all ignored lockdown channels"""
        try:
            ignored_channels = await self.get_ignored_channels(ctx.guild)
            if not ignored_channels:
                message = "No **channels** are being ignored during lockdowns"
                embed = make_embed_info(ctx.author, message)
                await ctx.send(embed=embed)
                return
            ...
            logger.error(f"Error in Channel lock_ignore_list: {e}")
            await ctx.message.add_reaction("👎")
    ...
    @commands.bot_has_permissions(manage_channels=True)
    @commands.has_permissions(manage_channels=True)
    async def unlock_channel(self, ctx: commands.Context, channel: Optional[discord.TextChannel]):
        """Unlock a channel"""
        default_role_id = await self.get_lock_role_id(ctx.guild)
        if default_role_id is None:
            default_role = ctx.guild.default_role
    ...
    async def nuke_channel(self, ctx: commands.Context):
        ...
        message = "Are you sure you want to **nuke** this **channel**?"
        embed = make_embed_warning(ctx.author, message)
...
#######################################################################################################
    ...
    async def force_nickname(self, ctx: commands.Context, member: discord.Member = None, *, new_nickname: typing.Optional[str] = None):
        ...
        if member == ctx.guild.owner:
            error_message = "You can't **forcenickname** the **owner**."
            error_embed = make_embed_error(ctx.author, error_message)
            await ctx.send(embed=error_embed)
            return
        ...
        try:
            ...
            logger.error(f"An error occurred while trying to count forced_nicknames: {e}")
            ...
            async with self.bot.pool.acquire() as conn:
                async with conn.cursor(aiomysql.DictCursor) as cur:
                    if new_nickname:
                        ...
                        success_message = f"Now **forcing nickname** for **{member}**"
                        success_embed = make_embed_success(ctx.author, success_message)
                    ...
                    # if new_nickname is None, check if a member has a nick already
                        ...
                        if member.display_name != member.name:
                            if member_has_forced_nickname is True:
                                ...
                                success_message = f"No longer **forcing nickname** for **{member}**"
                                success_embed = make_embed_success(ctx.author, success_message)
                                await ctx.send(embed=success_embed)
                                ...
                                await conn.commit()
                                success_message = f"Now **forcing current nickname** for **{member}**"
                                success_embed = make_embed_success(ctx.author, success_message)
                                await ctx.send(embed=success_embed)
                        else:
                            # if a member doesn't have a nickname, logger.error a warning message
                            warning_message = f"**{member}** has no nickname set to lock!"
                            warning_embed = make_embed_warning(ctx.author, warning_message)
                            await ctx.send(embed=warning_embed)
        except Exception as e:
            logger.exception(f"Error in {self.qualified_name} force_nickname: {e}")
    ...
    async def purge_messages(self, ctx: commands.Context, member: Optional[discord.Member], message_count: Optional[str] = 100):
        ...
        if message_count is not None:
            try:
                ...
                message = f"Provided argument **{message_count}** is not a number."
                embed = make_embed_error(ctx.author, message)
                await ctx.send(embed=embed)
                return
        ...
        if len(deleted) == 0:
            fail_message = "No **messages** could be deleted! (try a bigger search?)"
            embed = make_embed_warning(ctx.author, fail_message)
            await ctx.send(embed=embed)
    ...
    @commands.has_permissions(manage_messages=True)
    @commands.bot_has_permissions(manage_messages=True, read_message_history=True)
    async def purge_links(self, ctx: commands.Context, message_count: Optional[str] = 100):
    ...
    @commands.has_permissions(manage_messages=True)
    @commands.bot_has_permissions(manage_messages=True, read_message_history=True)
    async def purge_startswith(self, ctx: commands.Context, substring: str, message_count: Optional[str] = 100):
    ...
    @commands.has_permissions(manage_messages=True)
    @commands.bot_has_permissions(manage_messages=True, read_message_history=True)
    async def purge_bots(self, ctx: commands.Context, message_count: Optional[str] = 100):
    ...
    @commands.has_permissions(manage_messages=True)
    @commands.bot_has_permissions(manage_messages=True, read_message_history=True)
    async def purge_humans(self, ctx: commands.Context, message_count: Optional[str] = 100):
    ...
    @commands.has_permissions(manage_messages=True)
    @commands.bot_has_permissions(manage_messages=True, read_message_history=True)
    async def purge_contains(self, ctx: commands.Context, substring: str, message_count: Optional[str] = 100):
    ...
    @commands.has_permissions(manage_messages=True)
    @commands.bot_has_permissions(manage_messages=True, read_message_history=True)
    async def purge_files(self, ctx: commands.Context, message_count: Optional[str] = 100):
    ...
    @commands.has_permissions(manage_messages=True)
    @commands.bot_has_permissions(manage_messages=True, read_message_history=True)
    async def purge_images(self, ctx: commands.Context, message_count: str = None):
        ...
        if len(deleted) == 0:
            fail_message = "No **messages** could be deleted! (try a bigger search?)"
            embed = make_embed_warning(ctx.author, fail_message)
            fail_message = await ctx.send(embed=embed)
            await fail_message.delete(10)
    ...
    @commands.has_permissions(manage_messages=True)
    @commands.bot_has_permissions(manage_messages=True, read_message_history=True)
    async def purge_reactions(self, ctx: commands.Context, message_count: Optional[str] = 100):
    ...
    @commands.has_permissions(manage_messages=True)
    @commands.bot_has_permissions(manage_messages=True, read_message_history=True)
    async def purge_between(self, ctx: commands.Context, start_id: Optional[int], end_id: Optional[int]):
    ...
    @commands.has_permissions(manage_messages=True)
    @commands.bot_has_permissions(manage_messages=True, read_message_history=True)
    async def purge_endswith(self, ctx: commands.Context, substring: str, message_count: Optional[str] = 100):
    ...
    @commands.has_permissions(manage_messages=True)
    @commands.bot_has_permissions(manage_messages=True, read_message_history=True)
    async def purge_stickers(self, ctx: commands.Context, message_count: Optional[str] = 100):
    ...
    @commands.has_permissions(manage_messages=True)
    @commands.bot_has_permissions(manage_messages=True, read_message_history=True)
    async def purge_emoji(self, ctx: commands.Context, message_count: Optional[str] = 100):
    ...
    @commands.has_permissions(manage_messages=True)
    @commands.bot_has_permissions(manage_messages=True, read_message_history=True)
    async def purge_mentions(self, ctx: commands.Context, message_count: Optional[str] = 100):
    ...
    @commands.has_permissions(manage_messages=True)
    @commands.bot_has_permissions(manage_messages=True, read_message_history=True)
    async def purge_upto(self, ctx: commands.Context, messagelink: Optional[str]):
        """Purge messages up to a message link"""
        if messagelink is None:
            message = "Missing a **message link** to delete messages up to."
            embed = make_embed_warning(ctx.author, message)
            await ctx.send(embed=embed)
            return
    ...
    @commands.has_permissions(manage_messages=True)
    @commands.bot_has_permissions(manage_messages=True, read_message_history=True)
    async def purge_after(self, ctx: commands.Context, messagelink: Optional[str]):
        """Purge messages after a message link"""
        if messagelink is None:
            message = "Missing a **message link** to delete messages after."
            embed = make_embed_warning(ctx.author, message)
            await ctx.send(embed=embed)
            return
    ...
    @commands.has_permissions(manage_messages=True)
    @commands.bot_has_permissions(manage_messages=True, read_message_history=True)
    async def purge_before(self, ctx: commands.Context, messagelink: Optional[str]):
        """Purge messages before a message link"""
        if messagelink is None:
            message = "Missing a **message link** to delete messages before."
            embed = make_embed_warning(ctx.author, message)
            await ctx.send(embed=embed)
            return
    ...
    @commands.has_permissions(manage_messages=True)
    @commands.bot_has_permissions(manage_messages=True, read_message_history=True)
    async def purge_webhooks(self, ctx: commands.Context, message_count: Optional[str] = 100):
    ...
    @commands.has_permissions(manage_messages=True)
    @commands.bot_has_permissions(manage_messages=True, read_message_history=True)
    async def purge_embeds(self, ctx: commands.Context, message_count: Optional[str] = 100):
    ...
    async def ban_members(self, ctx: commands.Context, members: commands.Greedy[typing.Union[discord.Member, discord.User]] = None,
        ...
        date_time_dict = self.bot.misc_cog.return_delta_and_time_dict(delete_history)
        if date_time_dict is None:
            message = "Invalid delete history provided."
            embed = make_embed_error(ctx.author, message)
            await ctx.send(embed=embed)
            return
        ...
        if delete_seconds <= 0 or delete_seconds > 7 * 86400:
            message = "**Delete history** must be between `0` and `7` days."
            embed = make_embed_error(ctx.author, message)
            await ctx.send(embed=embed)
            return
        ...
        for member in members:
            ...
            if member == ctx.author:
                warning_message = "You can't **ban** yourself."
            ...
            if isinstance(member, discord.Member):
                ...
                if author_top_role <= target_top_role and not ctx.guild.owner:
                    warning_message = "You can't **ban** someone who is **higher** than you."
                    embed = make_embed_warning(ctx.author, warning_message)
                    await ctx.send(embed=embed)
                    continue
                ...
                if target_top_role >= bot_top_role:
                    warning_message = "I can't **ban** someone who is **higher** than me."
                    embed = make_embed_warning(ctx.author, warning_message)
                    await ctx.send(embed=embed)
                    continue
            try:
                ...
                error_message = f"The user with user ID **{member.id}** was not found."
                embed = make_embed_error(ctx.author, error_message)
                await ctx.send(embed=embed)
                continue
            except discord.Forbidden:
                error_message = "I **don't have** the permission to ban this user."
                embed = make_embed_error(ctx.author, error_message)
                await ctx.send(embed=embed)
                continue
            except discord.HTTPException as e:
                logging.exception(e)
                error_message = f"Failed to **ban** {member.mention}"
                embed = make_embed_error(ctx.author, error_message)
                await ctx.send(embed=embed)
                continue
        ...
        if len(banned_list) >= 1:
            ...
            guild_ban_message = f"Successfully **banned** {banned_string[:-2]}"
            embed = make_embed_success(ctx.author, guild_ban_message)
            await ctx.send(embed=embed)
    ...
    async def ban_purge(self, ctx: commands.Context, delete_history: Literal['6h', '12h', '24h', '3d', '7d', 'off']):
        ...
        try:
            ...
            message = f"Failed to set **message history** upon ban: {e}"
            embed = make_embed_error(ctx.author, message)
            await ctx.send(embed=embed) 
    ...
    @commands.has_guild_permissions(ban_members=True)
    @commands.bot_has_guild_permissions(ban_members=True)
    async def unban(self, ctx: commands.Context, user: discord.User = None, *, reason: typing.Optional[str] = "No reason provided"):
        ...
        try:
            ...
            if isinstance(entry, discord.BanEntry):
                try:
                    ...
                    message = f"Successfully unbanned **{user}**"
                    embed = make_embed_success(ctx.author, message)
                    await ctx.send(embed=embed)
                except discord.NotFound:
                    message = f"Couldn't find a ban for: **{user}**"
                    embed = make_embed_error(ctx.author, message)
                    await ctx.send(embed=embed)
                    return
                except discord.HTTPException:
                    message = f"HTTP: Couldn't find a ban for: **{user}**"
                    embed = make_embed_error(ctx.author, message)
                    await ctx.send(embed=embed)
                    return
        except discord.NotFound:
    ...
    async def kick(self, ctx: commands.Context, members: commands.Greedy[discord.Member] = None, *,
                   reason: typing.Optional[str] = "No reason provided"):
        ...
        for target in members:
            ...
            if target == ctx.author:
                warning_message = "You can't **kick** yourself."
                embed = make_embed_warning(ctx.author, warning_message)
                await ctx.send(embed=embed)
                continue

            if target == ctx.guild.owner:
                warning_message = "You can't **kick** **the owner**."
                embed = make_embed_warning(ctx.author, warning_message)
                await ctx.send(embed=embed)
                continue
            ...
            # Check if the bot is higher ranked than the target user
            if target_top_role >= bot_top_role:
                warning_message = "I can't **kick** someone who is **higher** than me."
                embed = make_embed_warning(ctx.author, warning_message)
                await ctx.send(embed=embed)
                continue
            ...
            if target_top_role >= author_top_role and not ctx.guild.owner:
                warning_message = "You can't **kick** someone who is **higher** than you."
                embed = make_embed_warning(ctx.author, warning_message)
                await ctx.send(embed=embed)
                continue
            ...
            try:
                ...
                error_message = f"Couldn't **kick** {target}"
                embed = make_embed_error(ctx.author, error_message)
                await ctx.send(embed=embed)
                continue
            except discord.HTTPException:
                error_message = f"Failed to **kick** {target}"
                embed = make_embed_error(ctx.author, error_message)
                await ctx.send(embed=embed)
                continue
        ...
        if len(kicked_list) >= 1:
            ...
            guild_kick_message = f"Successfully kicked {kicked_string[:-2]}"
            embed = make_embed_success(ctx.author, guild_kick_message)
            await ctx.send(embed=embed)
    ...
    async def timeout_member(self, ctx: commands.Context, members: commands.Greedy[discord.Member] = None,
        ...
        date_time, dict_keys, dict_values, dict_len, limit = self.bot.misc_cog.return_delta_and_time_dict(duration)
        ...
        for target in members:
            ...
            # Check if the command author is a target user
            if target == ctx.author:
                warning_message = "You can't **timeout** yourself."
                embed = make_embed_warning(ctx.author, warning_message)
                await ctx.send(embed=embed)
                continue

            if target == ctx.guild.owner:
                warning_message = "You can't **timeout** the **owner**."
                embed = make_embed_warning(ctx.author, warning_message)
                await ctx.send(embed=embed)
                continue
            ...
            # Check if the bot is higher ranked than the target user
            if target_top_role >= bot_top_role:
                warning_message = "I can't **timeout** someone who is **higher** than me."
                embed = make_embed_warning(ctx.author, warning_message)
                await ctx.send(embed=embed)
                continue
            ...
            if target.guild_permissions.administrator:
                warning_message = "I can't **timeout** someone who has the **Administrator** permission."
                embed = make_embed_warning(ctx.author, warning_message)
                await ctx.send(embed=embed)
                continue
            ...
            # Check if the command author is higher ranked than the target user
            if target_top_role >= author_top_role and not ctx.guild.owner:
                warning_message = "You can't **timeout** someone who is **higher** than you."
                embed = make_embed_warning(ctx.author, warning_message)
                await ctx.send(embed=embed)
                continue
            ...
            try:
                ...
                error_message = "`TypeError`: The **duration** parameter is the wrong type."
                embed = make_embed_error(ctx.author, error_message)
                await ctx.send(embed=embed)
                continue
            except discord.Forbidden:
                error_message = "I can't **timeout** someone who has the **Administrator** permission."
                embed = make_embed_error(ctx.author, error_message)
                await ctx.send(embed=embed)
                continue
            except discord.HTTPException:
                error_message = f"`HTTPException`: Failed to **timeout {target}**."
                embed = make_embed_error(ctx.author, error_message)
                await ctx.send(embed=embed)
                continue
        ...
        if len(timeout_list) >= 1:
            ...
            if dict_len == 1:
                key0 = self.bot.misc_cog.singular_plural(dict_keys[0], dict_values[0])
                timeout_message = f"{timeout_string[:-2]} timed out for **{dict_values[0]} {key0}**."
                embed = make_embed_success(ctx.author, timeout_message)
                await ctx.send(embed=embed)
            elif dict_len == 2:
                key0 = self.bot.misc_cog.singular_plural(dict_keys[0], dict_values[0])
                key1 = self.bot.misc_cog.singular_plural(dict_keys[1], dict_values[1])
                ...
                key0 = self.bot.misc_cog.singular_plural(dict_keys[0], dict_values[0])
                key1 = self.bot.misc_cog.singular_plural(dict_keys[1], dict_values[1])
                key2 = self.bot.misc_cog.singular_plural(dict_keys[2], dict_values[2])
    ...
    @commands.bot_has_guild_permissions(moderate_members=True)
    @commands.has_guild_permissions(moderate_members=True)
    async def remove_timeout(self, ctx: commands.Context, members: commands.Greedy[discord.Member] = None, *,
                             reason: typing.Optional[str] = "No reason provided"):
        ...
        for target in members:
            ...
            if isinstance(target, discord.Member):
                ...
                if target_top_role == bot_top_role:
                    warning_message = "I can't **remove a timeout** from a member with **equal rank** as mine"
                    embed = make_embed_warning(ctx.author, warning_message)
                    await ctx.send(embed=embed)
                    continue

                if target_top_role > bot_top_role:
                    warning_message = "I can't **remove a timeout** from a member **higher** than me"
                    embed = make_embed_warning(ctx.author, warning_message)
                    await ctx.send(embed=embed)
                    continue

                if target_top_role > author_top_role and not ctx.guild.owner:
                    warning_message = "You can't **remove a timeout** from a member **higher** than you"
                    embed = make_embed_warning(ctx.author, warning_message)
                    await ctx.send(embed=embed)
                    continue
            ...
            if target.is_timed_out():
                ...
                try:
                    ...
                    error_message = f"`Forbidden`: I don't have proper permissions to **remove a timeout** from **{target}**."
                    embed = make_embed_error(ctx.author, error_message)
                    await ctx.send(embed=embed)
                    continue
                except discord.HTTPException:
                    error_message = f"`HTTPException`: Failed to **remove a timeout** from **{target}**."
                    embed = make_embed_error(ctx.author, error_message)
                    await ctx.send(embed=embed)
                    continue
            else:
                error_message = f"**{target}** isn't timed out"
        ...
        if len(untime_list) >= 1:
            ...
            untime_message = f"Successfully **removed a timeout** from {untime_string[:-2]}"
            embed = make_embed_success(ctx.author, untime_message)
            await ctx.send(embed=embed)
        ...
        if len(not_timed_out_list) >= 1:
            ...
            untime_message = f"{untime_string[:-2]} not timed out."
            embed = make_embed_error(ctx.author, untime_message)
            await ctx.send(embed=embed)
    ...
    @commands.has_guild_permissions(administrator=True)
    async def strip_staff(self, ctx: commands.Context, member: discord.Member = None):
        ...
        if member:
            ...
            if member == ctx.author:
                warning_message = "You can't **strip staff perms** from yourself."
                embed = make_embed_warning(ctx.author, warning_message)
                await ctx.send(embed=embed)
                return

            if member == ctx.guild.owner:
                warning_message = "You can't **strip staff perms** from **the owner**."
                embed = make_embed_warning(ctx.author, warning_message)
                await ctx.send(embed=embed)
                return

            if member.top_role >= ctx.author.top_role:
                warning_message = "You can't **strip staff perms** from a member **higher** than you."
                embed = make_embed_warning(ctx.author, warning_message)
                await ctx.send(embed=embed)
                return
            ...
            for role in member.roles:
                if check_role_perms(role) and role.name != "@everyone":
                    ...
                    try:
                        ...
                        error_message = "I don't have the permission to remove these roles."
                        embed = make_embed_error(ctx.author, error_message)
                        await ctx.send(embed=embed)
                        continue
                    except discord.NotFound as e:
                        error_message = f"An error occurred while removing roles: {e}"
                        embed = make_embed_error(ctx.author, error_message)
                        await ctx.send(embed=embed)
                        continue
            ...
            if len(roles_fail_bot) > 0 or len(roles_fail_author) > 0:
                ...
                for role in roles_fail_author:
                    failed_string += f"{role.mention}, "
                fail_message = f"Failed to remove {failed_string[:-2]}, user hasn't been stripped completely."
                embed = make_embed_error(ctx.author, fail_message)
                await ctx.send(embed=embed)
                return
            ...
            if len(removed_roles) > 0:
                ...
                success_message = f"No staff perms found in {member.mention}'s roles."
                embed = make_embed_warning(ctx.author, success_message)
                await ctx.send(embed=embed)
        else:
    ...
    async def assign_role(self, ctx: commands.Context, members: commands.Greedy[discord.Member] = None, *, role_name: str = None):
        ...
        if members is None and role_name is None:
            ...
            return
        if members is None:
            warning_message = "Missing a **role** to **add/remove to/from** member"
            embed = make_embed_warning(ctx.author, warning_message)
            await ctx.send(embed=embed)
            return
        ...
        try:
            ...
            if len(roles_containing_target_name) == 0:
                warning_message = f"I was unable to find a role with the name: **{role_name}**"
                embed = make_embed_warning(ctx.author, warning_message)
                await ctx.send(embed=embed)
                return
            ...
            try:
                ...
                error_message = f"I was unable to find a role with the name: **{role_name}**"
                embed = make_embed_error(ctx.author, error_message)
                await ctx.send(embed=embed)
                return
        except TypeError:
            error_message = "Missing a **role** to **add/remove to/from** member"
            embed = make_embed_error(ctx.author, error_message)
            await ctx.send(embed=embed)
            return
        ...
        if role not in guild_roles:
            warning_message = f"I was unable to find a role with the name: **{role}**"
            embed = make_embed_warning(ctx.author, warning_message)
            await ctx.send(embed=embed)
            return
        ...
        if not role.is_assignable():
            warning_message = f"{role.mention} role **isn't assignable**."
            embed = make_embed_warning(ctx.author, warning_message)
            await ctx.send(embed=embed)
            return
        ...
        if role >= ctx.guild.me.top_role:
            warning_message = f"**{role}** is above my **role** - please drag mine above!"
            embed = make_embed_warning(ctx.author, warning_message)
            await ctx.send(embed=embed)
            return
        ...
        if role >= ctx.author.top_role and not ctx.guild.owner.id:
            warning_message = f"{role.mention} is **too high** for you to manage"
            embed = make_embed_warning(ctx.author, warning_message)
            await ctx.send(embed=embed)
            return
        ...
        for member in members:
            ...
            if role_check is None:  # means the member doesn't have that role
                try:
                    ...
                    error_message = f"Forbidden to give role {role.mention} to {member.mention}"
                    embed = make_embed_error(ctx.author, error_message)
                    await ctx.send(embed=embed)
                    continue
                except discord.HTTPException:
                    error_message = f"HTTPException while trying to give role {role.mention} to {member.mention}"
                    embed = make_embed_error(ctx.author, error_message)
                    await ctx.send(embed=embed)
                    continue
                ...
                try:
                    ...
                    error_message = f"Forbidden to remove role {role.mention} from {member.mention}"
                    embed = make_embed_error(ctx.author, error_message)
                    await ctx.send(embed=embed)
                    continue
                except discord.HTTPException:
                    error_message = f"HTTPException while trying to remove role {role.mention} from {member.mention}"
                    embed = make_embed_error(ctx.author, error_message)
                    await ctx.send(embed=embed)
                    continue
        ...
        if len(added_list) >= 1:
            ...
            logger.info(f"Added {role.mention} to {added_string}")
            await ctx.send(embed=embed)
    ...
    @commands.bot_has_guild_permissions(manage_roles=True)
    @commands.has_guild_permissions(manage_roles=True)
    async def create_new_role(self, ctx: commands.Context, role_color: Union[discord.Color | str] = None, *, role_name: str = None):
        ...
        try:
            ...
            success_message = f"Successfully created {created_role.mention}"
            embed = make_embed_success(ctx.author, success_message)
            await ctx.send(embed=embed)
        except Exception as e:
            await ctx.send(f"Error creating a role: {e}")
    ...
    @commands.bot_has_guild_permissions(manage_roles=True)
    @commands.has_guild_permissions(manage_roles=True)
    async def delete_role(self, ctx: commands.Context, roles: commands.Greedy[discord.Role] = None):
        ...
        for role in roles:
            if role >= ctx.guild.me.top_role:
                message = "I can't delete a role higher than my top role."
                embed = make_embed_warning(ctx.author, message)
                await ctx.send(embed=embed)
                return

            if role >= ctx.author.top_role and ctx.author != ctx.guild.owner:
                message = "You can't delete a role higher than your top role."
                embed = make_embed_warning(ctx.author, message)
                await ctx.send(embed=embed)
                return

            if role.is_bot_managed():
                message = "I can't delete bot managed roles."
                embed = make_embed_warning(ctx.author, message)
                await ctx.send(embed=embed)
                return
            ...
            try:
                ...
                message = f"I don't have proper permissions to delete {role.mention}"
                embed = make_embed_error(ctx.author, message)
                await ctx.send(embed=embed)
                return
            except discord.HTTPException:
                message = f"`HTTPException`: Deleting the role {role.mention} failed."
                embed = make_embed_error(ctx.author, message)
                await ctx.send(embed=embed)
                return

        message = f"Successfully **deleted** specified roles."
        embed = make_embed_success(ctx.author, message)
        await ctx.send(embed=embed)
    ...
    async def _silence(self, ctx: commands.Context, member: typing.Optional[discord.Member]):
        ...
        try:
            ...
            async with self.bot.pool.acquire() as conn:
                async with conn.cursor(aiomysql.DictCursor) as cur:
                    ...
                    if cur_result:
                        message = f"{member.mention} is already **silenced**"
                        embed = make_embed_error(ctx.author, message)
                        await ctx.send(embed=embed)
                        return
                    else:
                ...
                await conn.commit()
            message = f"{member.mention} is being **silenced** from now on"
            embed = make_embed_success(ctx.author, message)
            await ctx.send(embed=embed)
        except Exception as e:
            logger.error(f"An error has occurred in {self.qualified_name} database silence check: {e}")
            message = f"An error has occurred in {self.qualified_name} database silence check: {e}"
            error_embed = make_embed_error(ctx.author, message)
            await ctx.send(embed=error_embed)
            return
    ...
    @commands.has_guild_permissions(moderate_members=True)
    async def unsilence(self, ctx: commands.Context, member: typing.Optional[discord.Member]):
            ...
            async with self.bot.pool.acquire() as conn:
                async with conn.cursor(aiomysql.DictCursor) as cur:
                    ...
                    if cur_result:
                        ...
                        message = f"{member.mention} is no longer being **silenced**"
                        embed = make_embed_success(ctx.author, message)
                        await ctx.send(embed=embed)
                        await conn.commit()
                        return
                    else:
                        message = f"{member.mention} is not being **silenced**"
                        embed = make_embed_error(ctx.author, message)
                        await ctx.send(embed=embed)
        except Exception as e:
            logger.error(f"An error has occurred in {self.qualified_name} database silence check: {e}")
    ...
    async def warn(self, ctx: commands.Context, member: discord.Member, *, reason: Optional[str]):

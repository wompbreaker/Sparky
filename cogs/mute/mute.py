...
class Mute(commands.Cog):
    ...
    async def setup_mute_member(self, ctx: commands.Context):
    ...
    async def mute_member(self, ctx: commands.Context, member: discord.Member = None, *,
        ...
        try:
            ...
            logger.error(f"An error has occurred in {self.qualified_name} database mute check: {e}")
            message = f"An error has occurred in {self.qualified_name} database mute check: {e}"
            error_embed = make_embed_error(ctx.author, message)
        ...
        try:
            if muted_role_exists is False:
                warning_message = f"No **muted** role was found, please run the `{cmd_prefix}setupmute` command first"
                warning_embed = make_embed_warning(ctx.author, warning_message)
                await ctx.send(embed=warning_embed)
                return

            if muted_role >= ctx.guild.me.top_role:
                message = "The **muted role** is higher than my top role! Please **move it below my top role**."
            ...
            if member == ctx.guild.owner and ctx.author != ctx.guild.owner:
                message = "You can't **unmute the owner**."
                embed = make_embed_warning(ctx.author, message)
                await ctx.send(embed=embed)
                return
            ...
            if member.guild_permissions.administrator:
                warning_message = f"**{member}** has administrator permission so this won't work"
                warning_embed = make_embed_warning(ctx.author, warning_message)
                await ctx.send(embed=warning_embed)
                return
            # Check if the target member is higher than author
            if member.top_role >= ctx.author.top_role and ctx.author != ctx.guild.owner:
                message = "You can't **unmute** someone **equal or higher** than you."
                embed = make_embed_warning(ctx.author, message)
                await ctx.send(embed=embed)
                return
            ...
            if role_check is None:
                ...
                error_message = f"**{member}** is already muted"
                embed = make_embed_error(ctx.author, error_message)
                await ctx.send(embed=embed)
                return

            success_message = f"Successfully **muted** {member.mention}\nReason: *{reason}*"
            embed = make_embed_mute(ctx.author, success_message)
            await ctx.send(embed=embed)
        except Exception as e:
            logger.error(f"Error in {self.qualified_name} mute_member: {e}")

    @commands.command(name='imute')
    @commands.bot_has_guild_permissions(manage_roles=True)
    @commands.has_guild_permissions(moderate_members=True)
    async def image_mute_member(self, ctx: commands.Context, member: discord.Member = None, *,
                                reason: typing.Optional[str] = "No reason provided"):
        ...
        if member is None and reason == "No reason provided":
            ...
            usage = f"```Syntax: {cmd_prefix}imute (member) (reason)\nExample: {cmd_prefix}imute anakin spam```"
            embed = make_embed_help(ctx, command_name, description, usage)
            await ctx.send(embed=embed)
            return
        ...
        try:
            ...
            logger.error(f"An error has occurred in {self.qualified_name} database imute check: {e}")
            message = f"An error has occurred in {self.qualified_name} database imute check: {e}"
            error_embed = make_embed_error(ctx.author, message)
            await ctx.send(embed=error_embed)
            return
        ...
        try:
            if imuted_role_exists is False:
                warning_message = f"No **imuted** role was found, please run the `{cmd_prefix}setupmute` command first"
                warning_embed = make_embed_warning(ctx.author, warning_message)
                await ctx.send(embed=warning_embed)
                return

            if imuted_role >= ctx.guild.me.top_role:
                message = "The **imuted role** is higher than my top role! Please **move it below my top role**."
                embed = make_embed_warning(ctx.author, message)
                await ctx.send(embed=embed)
                return
            ...
            if member == ctx.author and imuted_role >= ctx.author.top_role and ctx.author != ctx.guild.owner:
                message = "You can't **imute** yourself."
                embed = make_embed_warning(ctx.author, message)
                await ctx.send(embed=embed)
                return

            if member == ctx.guild.owner and ctx.author != ctx.guild.owner:
                message = "You can't **imute the owner**."
                embed = make_embed_warning(ctx.author, message)
                await ctx.send(embed=embed)
                return
            ...
            # Check if the target member is higher than author
            if member.top_role >= ctx.author.top_role and ctx.author != ctx.guild.owner:
                message = "You can't **imute** someone **equal or higher** than you."
                embed = make_embed_warning(ctx.author, message)
                await ctx.send(embed=embed)
                return
            ...
            if role_check is None:  # means the member doesn't have that role
                ...
                error_message = f"**{member}** is already imuted"
                embed = make_embed_error(ctx.author, error_message)
                await ctx.send(embed=embed)
                return
            ...
            logger.error(f"Error in {self.qualified_name} image_mute_member: {e}")

    @commands.command(name='rmute')
    @commands.bot_has_guild_permissions(manage_roles=True)
    @commands.has_guild_permissions(moderate_members=True)
    async def reaction_mute_member(self, ctx: commands.Context, member: discord.Member = None, *,
                                   reason: typing.Optional[str] = "No reason provided"):
        ...
        if member is None and reason == "No reason provided":
            ...
            usage = f"```Syntax: {cmd_prefix}rmute (member) (reason)\nExample: {cmd_prefix}rmute anakin spam```"
            embed = make_embed_help(ctx, command_name, description, usage)
            await ctx.send(embed=embed)
            return
        ...
        try:
            ...
            logger.error(f"An error has occurred in {self.qualified_name} database rmute check: {e}")
            message = f"An error has occurred in {self.qualified_name} database rmute check: {e}"
            error_embed = make_embed_error(ctx.author, message)
            await ctx.send(embed=error_embed)
            return
        ...
        try:
            if rmuted_role_exists is False:
                warning_message = f"No **rmuted** role was found, please run the `{cmd_prefix}setupmute` command first"
                warning_embed = make_embed_warning(ctx.author, warning_message)
                await ctx.send(embed=warning_embed)
                return

            if rmuted_role >= ctx.guild.me.top_role:
                message = "The **rmuted role** is higher than my top role! Please **move it below my top role**."
                embed = make_embed_warning(ctx.author, message)
                await ctx.send(embed=embed)
                return
            ...
            if member == ctx.author and rmuted_role >= ctx.author.top_role and ctx.author != ctx.guild.owner:
                message = "You can't **rmute** yourself."
                embed = make_embed_warning(ctx.author, message)
                await ctx.send(embed=embed)
                return

            if member == ctx.guild.owner and ctx.author != ctx.guild.owner:
                message = "You can't **rmute the owner**."
                embed = make_embed_warning(ctx.author, message)
                await ctx.send(embed=embed)
                return
            ...
            if role_check is None:  # means the member doesn't have that role
                ...
                error_message = f"**{member}** is already rmuted"
                embed = make_embed_error(ctx.author, error_message)
                await ctx.send(embed=embed)
                return
            ...
            logger.error(f"Error in {self.qualified_name} reaction_mute_member: {e}")
    ...
    async def unmute_member(self, ctx: commands.Context, member: discord.Member = None, *,
        ...
        if member is None and reason == "No reason provided":
            ...
            usage = f"```Syntax: {cmd_prefix}unmute (member) (reason)\nExample: {cmd_prefix}unmute anakin being nice again```"
            embed = make_embed_help(ctx, command_name, description, usage)
            await ctx.send(embed=embed)
            return
        ...
        try:
            ...
            logger.error(f"An error has occurred in {self.qualified_name} database unmute check: {e}")
            message = f"An error has occurred in {self.qualified_name} database unmute check: {e}"
            error_embed = make_embed_error(ctx.author, message)
            await ctx.send(embed=error_embed)
            return
        ...
        try:
            ...
            if role_check is not None:  # means the member has that role
                ...
                error_message = f"**{member}** is not muted"
                embed = make_embed_error(ctx.author, error_message)
                await ctx.send(embed=embed)
                return
            ...
            logger.error(f"Error in {self.qualified_name} unmute_member: {e}")

    @commands.command(name='iunmute')
    @commands.bot_has_guild_permissions(manage_roles=True)
    @commands.has_guild_permissions(moderate_members=True)
    async def image_unmute_member(self, ctx: commands.Context, member: discord.Member = None, *,
                                  reason: typing.Optional[str] = "No reason provided"):

        # display help embed if command parameters are invalid
        cmd_prefix = await self.bot.bot_prefix(ctx.guild)
        if member is None and reason == "No reason provided":
            command_name = "iunmute"
            description = "Restores a member's attach files & embed links permission"
            usage = f"```Syntax: {cmd_prefix}iunmute (member) (reason)\nExample: {cmd_prefix}iunmute anakin no more spam```"
            embed = make_embed_help(ctx, command_name, description, usage)
            await ctx.send(embed=embed)
            return
        ...
        try:
            if imuted_role_exists is False:
                warning_message = "No **imuted** role was found, please run the `,setupmute` command first"
                warning_embed = make_embed_warning(ctx.author, warning_message)
                await ctx.send(embed=warning_embed)
                return
            ...
            # Check if the target member is the command author
            if member == ctx.author and imuted_role >= ctx.author.top_role and ctx.author != ctx.guild.owner:
                message = "You can't **iunmute** yourself."
                embed = make_embed_warning(ctx.author, message)
                await ctx.send(embed=embed)
                return

            if member == ctx.guild.owner and ctx.author != ctx.guild.owner:
                message = "You can't **iunmute the owner**."
                embed = make_embed_warning(ctx.author, message)
                await ctx.send(embed=embed)
                return
            ...
            # Check if the target member is higher than author
            if member.top_role >= ctx.author.top_role and ctx.author != ctx.guild.owner:
                message = "You can't **iunmute** someone **equal or higher** than you."
                embed = make_embed_warning(ctx.author, message)
                await ctx.send(embed=embed)
                return
            ...
            if role_check is not None:  # means the member has that role
                ...
                error_message = f"**{member}** is not imuted"
                embed = make_embed_error(ctx.author, error_message)
                await ctx.send(embed=embed)
                return
            ...
            logger.error(f"Error in {self.qualified_name} image_unmute_member: {e}")

    @commands.command(name='runmute')
    @commands.bot_has_guild_permissions(manage_roles=True)
    @commands.has_guild_permissions(moderate_members=True)
    async def reaction_unmute_member(self, ctx: commands.Context, member: discord.Member = None, *,
                                     reason: typing.Optional[str] = "No reason provided"):

        # display help embed if command parameters are invalid
        cmd_prefix = await self.bot.bot_prefix(ctx.guild)
        if member is None and reason == "No reason provided":
            command_name = "runmute"
            description = "Restores a member's add reactions & use external emotes permission"
            usage = f"```Syntax: {cmd_prefix}runmute (member) (reason)\nExample: {cmd_prefix}runmute anakin no more spam```"
            embed = make_embed_help(ctx, command_name, description, usage)
            await ctx.send(embed=embed)
            return
        ...
        try:
            ...
            logger.error(f"An error has occurred in {self.qualified_name} database runmute check: {e}")
            message = f"An error has occurred in {self.qualified_name} database runmute check: {e}"
            error_embed = make_embed_error(ctx.author, message)
            await ctx.send(embed=error_embed)
            return
        ...
        try:
            ...
            # Check if the target member is the command author
            if member == ctx.author and rmuted_role >= ctx.author.top_role and ctx.author != ctx.guild.owner:
                message = "You can't **runmute** yourself."
                embed = make_embed_warning(ctx.author, message)
                await ctx.send(embed=embed)
                return

            if member == ctx.guild.owner and ctx.author != ctx.guild.owner:
                message = "You can't **runmute the owner**."
                embed = make_embed_warning(ctx.author, message)
                await ctx.send(embed=embed)
                return
            ...
            # Check if the target member is higher than author
            if member.top_role >= ctx.author.top_role and ctx.author != ctx.guild.owner:
                message = "You can't **runmute** someone **equal or higher** than you."
                embed = make_embed_warning(ctx.author, message)
                await ctx.send(embed=embed)
                return
            # Try to mute members
            ...
            if role_check is not None:  # means the member has that role
                ...
                error_message = f"**{member}** is not rmuted"
                embed = make_embed_error(ctx.author, error_message)
                await ctx.send(embed=embed)
                return
            ...
            logger.error(f"Error in {self.qualified_name} runmute_member: {e}")
